...
<div id="formulairephoto">
  ...
  <form class="col-md-6 jumbotron" action="<?= $this->url('photo_add_photos') ?>" method="post" enctype="multipart/form-data" id="ajoutphotoform">
    ...
    <div class="form-group text-center">
        <label for="id_gallery" class="control-label">Catégorie de gallery</label>
        <div>
           <select id="id_gallery" name="id_gallery" class="form-control" required>
              <option disabled selected required>-- Sélectionner gallery --</option>
              <option value="1" id="id_gallery" required>Photo gallery</option>
              <option value="2" id="id_gallery" required>Photo gallery</option>
            </select>
        </div>
    </div>
    <!-- Ajout photo -->
